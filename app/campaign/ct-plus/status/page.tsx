...
const CTPlusOverviewTab = dynamic(
  () => import("../overview/page"),
  { ssr: false, loading: () => <div className="p-8 text-center text-sm text-gray-500">로딩 중…</div> }
)
  ...
  async function handleAgencyExcelUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const newAgencies = data.map(row => ({
        ...
        name:             String(row["대행사명"]             ?? ""),
        contactName:      String(row["담당자명"]             ?? ""),
        email:            String(row["이메일"]               ?? ""),
        phone:            String(row["전화번호"]             ?? ""),
        corporateName:    row["법인명"]             ? String(row["법인명"])             : undefined,
        businessNumber:   row["사업자등록번호"]     ? String(row["사업자등록번호"])     : undefined,
        representative:   row["대표자명"]           ? String(row["대표자명"])           : undefined,
        address:          row["주소"]               ? String(row["주소"])               : undefined,
        businessType:     row["업태"]               ? String(row["업태"])               : undefined,
        businessItem:     row["종목"]               ? String(row["종목"])               : undefined,
        defaultMarkupRate:row["기본수수료율(%)"]    ? parseFloat(String(row["기본수수료율(%)"])) : undefined,
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <header className="border-b border-gray-200 bg-white px-6 py-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            {statusTab === "operators" && (
              <button onClick={() => { setEditOp(null); setOpModalOpen(true) }} className={btnPrimary}>
                + 운영자 추가
              </button>
            )}
            {statusTab === "agencies" && agencySubTab === "list" && (
              <div className="flex gap-2">
                <button onClick={() => { setEditAg(null); setAgencySubTab("edit") }} className={btnPrimary}>
                  + 새 대행사
                </button>
                ...
                <input ref={fileInputRef} type="file" accept=".xlsx,.xls"
                  onChange={handleAgencyExcelUpload} className="hidden" />
              </div>
            )}
            {statusTab === "advertisers" && (
              <button onClick={() => { setEditAdv(null); setAdvModalOpen(true) }} className={btnPrimary}>
                + 광고주 추가
              </button>
            )}
          </div>
        ...
        <div className="mt-3 flex gap-1">
          {([
            ...
            <button
              key={tab}
              onClick={() => { setStatusTab(tab); if (tab === "agencies") setAgencySubTab("list") }}
              className={`rounded-lg px-4 py-1.5 text-sm font-medium transition-colors ${
                statusTab === tab ? "bg-blue-600 text-white" : "text-gray-500 hover:bg-gray-100"
              }`}
            >
              {label}
            </button>
      ...
      <main className="p-6 space-y-4">
        ...
        {statusTab === "agencies" && (
          <>
            {agencySubTab === "list" && (
              <AgencyListTab
                agencies={agencies}
                onEdit={(ag) => { setEditAg(ag); setAgencySubTab("edit") }}
                onDelete={handleDeleteAg}
              />
            )}
            {agencySubTab === "edit" && (
              <AgencyEditTab
                agency={editAg} agencies={agencies}
                onSave={(ag) => {
                  if (editAg) saveAgencies(agencies.map(a => a.id === ag.id ? ag : a))
                  else        saveAgencies([...agencies, ag])
        ...
        {statusTab === "advertisers" && (
          <AdvertiserListTab
            advertisers={advertisers} agencies={agencies} campaigns={campaigns}
            onEdit={(adv) => { setEditAdv(adv); setAdvModalOpen(true) }}
            onDelete={handleDeleteAdv}
          />
        )}

        {statusTab === "operators" && (
          <OperatorListTab
            operators={operators} campaigns={campaigns}
            onEdit={(op) => { setEditOp(op); setOpModalOpen(true) }}
      ...
      {opModalOpen && (
        <OperatorModal
          open={opModalOpen} onClose={() => setOpModalOpen(false)}
          editOp={editOp} operators={operators}
          onSave={(op) => {
            if (editOp) saveOperators(operators.map(o => o.id === op.id ? op : o))
            else        saveOperators([...operators, { ...op, id: Date.now().toString() }])
      ...
      {advModalOpen && (
        <AdvertiserModal
          open={advModalOpen} onClose={() => setAdvModalOpen(false)}
          editAdv={editAdv} agencies={agencies}
          onSave={(adv) => {
            if (editAdv) saveAdvertisers(advertisers.map(a => a.id === adv.id ? adv : a))
            else         saveAdvertisers([...advertisers, { ...adv, id: Date.now().toString() }])
      ...
      {agModalOpen && (
        <AgencyFormModal
          open={agModalOpen} onClose={() => setAgModalOpen(false)}
          editAg={editAg}
          onSave={(ag) => {
            if (editAg) saveAgencies(agencies.map(a => a.id === ag.id ? ag : a))
            else        saveAgencies([...agencies, { ...ag, id: Date.now().toString() }])
